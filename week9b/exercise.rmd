...
1. research hypothesis ($H_1$): 花錢會增加人們繼續看電影的意願。
2. null hypothesis ($H_0$): 不論有沒有花錢，人們繼續看電影的意願都相同。
3. 取得有花錢的人的樣本（樣本統計量為 $M$）。
4. 取得沒花錢的人的 sampling distribution（$H_0$ 為真）。
5. 計算在 $H_0$ 為真的情況下，樣本統計量不小於 $M$ 的機率（p 值）。
